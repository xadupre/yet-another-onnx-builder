"""
.. _l-plot-input-observer-onnx-diagnostic:

InputObserver from onnx-diagnostic
====================================

:epkg:`onnx-diagnostic` ships its own
:class:`InputObserver <onnx_diagnostic.investigate.input_observer.InputObserver>`
which has the same context-manager API as
:class:`yobx.torch.input_observer.InputObserver` but lives in a separate package.
This makes it easy to collect representative inputs and infer dynamic shapes
**without** having yobx as a dependency.

The example below mirrors the three scenarios from
:ref:`l-plot-input-observer` so you can compare both implementations
side-by-side:

1. **Positional args** — two plain tensors whose batch and sequence lengths vary
   across calls.
2. **Keyword args** — same model, but inputs are passed as named arguments.
3. **Optional argument** — a model where one input (``mask``) is absent in some
   calls; ``value_if_missing`` provides the substitute tensor so that dynamic
   shape analysis can still be performed.
"""
...
#
# :class:`InputObserver <onnx_diagnostic.investigate.input_observer.InputObserver>`
# works identically when the model is called with keyword arguments.
# The inferred dynamic shapes are returned as a *dict* (one entry per named
...
# Three calls with varying batch size; feature dim is fixed.
kwargs_lin = [
...
ax.set_ylim(0, 5)
ax.axis("off")
ax.set_title("InputObserver (onnx-diagnostic) workflow", fontsize=12)
