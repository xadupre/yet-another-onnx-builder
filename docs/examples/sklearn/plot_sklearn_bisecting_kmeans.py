"""
.. _l-plot-sklearn-bisecting-kmeans:

Converting a scikit-learn BisectingKMeans to ONNX
===================================================

:func:`yobx.sklearn.to_onnx` converts a fitted
:class:`sklearn.cluster.BisectingKMeans` into an
...
* **label** - cluster index for each sample (equivalent to
  :meth:`~sklearn.cluster.BisectingKMeans.predict`).
* **distances** - Euclidean distance from each sample to every centroid
  (equivalent to :meth:`~sklearn.cluster.BisectingKMeans.transform`).

The workflow is:

1. **Train** a :class:`~sklearn.cluster.BisectingKMeans` as usual.
...
# %%
# 1. Train a BisectingKMeans model
# ---------------------------------
...
label_sk = bkm.predict(X).astype(np.int64)
distances_sk = bkm.transform(X).astype(np.float32)
...
# %%
# 4. BisectingKMeans inside a Pipeline
# -------------------------------------
#
# BisectingKMeans also works as the final step of a
