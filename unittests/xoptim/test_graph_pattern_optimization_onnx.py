...
class TestGraphPatternOptimization(ExtTestCase):
    ...
    def test_expand_unsqueeze_expand(self):
        model = oh.make_model(
            oh.make_graph(
                ...
                "dummy",
                [_mkv_("X", TFLOAT, [1, "a", "b"])],
                [_mkv_("Y", TFLOAT, [3, 4, "a", "b"])],
                [
                    onh.from_array(np.array([3, 1, 1], dtype=np.int64), name="shape1"),
                    onh.from_array(np.array([1], dtype=np.int64), name="axes"),
                    onh.from_array(np.array([3, 4, 1, 1], dtype=np.int64), name="shape2"),
                ],
            ),
            opset_imports=[oh.make_opsetid("", 18)],
            ir_version=10,
    ...
    def test_mul_unsqueeze_expand(self):
        # Mul(x, Unsqueeze(Expand(y, shape), axis)) where x covers the expanded dims.
        # y: (1, a, b), Expand to (c, a, b), Unsqueeze at 1 -> (c, 1, a, b)
        # x: (c, d, a, b)  -- x.dim[0]==c, so Expand is redundant.
        model = oh.make_model(
            oh.make_graph(
                [
                    oh.make_node("Expand", ["Y", "shape"], ["ye"]),
                    oh.make_node("Unsqueeze", ["ye", "axes"], ["yu"]),
                    oh.make_node("Mul", ["X", "yu"], ["Z"]),
                ],
                "dummy",
                [
        ...
        opt_onx = gr.to_onnx(optimize=True)
        self.assertEqual(["Unsqueeze", "Mul"], [n.op_type for n in opt_onx.graph.node])

        opt_ref = ExtendedReferenceEvaluator(opt_onx)
        got = opt_ref.run(None, feeds)[0]
        self.assertEqualArray(expected, got)

    def test_mul_unsqueeze_expand_no_match_x_dim_one(self):
        # Expand is NOT redundant: x.dim[0]==1 so broadcasting in Mul would not
        # replicate the dimension that Expand introduced (c).
        model = oh.make_model(
            oh.make_graph(
                [
                    oh.make_node("Expand", ["Y", "shape"], ["ye"]),
        ...
        opt_onx = gr.to_onnx(optimize=True)
        # Pattern should NOT fire because x.dim[0]==1 != shape[0]==3.
        self.assertEqual(
            ["Expand", "Unsqueeze", "Mul"], [n.op_type for n in opt_onx.graph.node]
        )
        ref2 = ExtendedReferenceEvaluator(opt_onx)
        got = ref2.run(None, feeds)[0]
        self.assertEqualArray(expected, got)

    def test_mul_unsqueeze_expand_no_match_expand_shared(self):
        # Pattern should NOT fire: Expand output is used by both Unsqueeze and Relu.
        model = oh.make_model(
            oh.make_graph(
                [
                    oh.make_node("Expand", ["Y", "shape"], ["ye"]),
                ...
                [_mkv_("Z", TFLOAT, [3, 4, 5, 7]), _mkv_("W", TFLOAT, [3, 5, 7])],
                [
                    onh.from_array(np.array([3, 5, 7], dtype=np.int64), name="shape"),
                    onh.from_array(np.array([1], dtype=np.int64), name="axes"),
                ],
    ...
    def test_mul_unsqueeze_unsqueeze_1(self):
        model = oh.make_model(
            oh.make_graph(
                [
                    oh.make_node("Unsqueeze", ["X", "axes"], ["xu"]),
                    oh.make_node("Unsqueeze", ["Y", "axes"], ["yu"]),
                    oh.make_node("Mul", ["xu", "yu"], ["Z"]),
                ],
                "test",
                [
        ...
        feeds = {
            ...
            "Y": np.arange(12).reshape(3, 4).astype(np.float32) + 1,
        }
        ref = ExtendedReferenceEvaluator(model)
        z = ref.run(None, feeds)[0]
    ...
    def test_mul_unsqueeze_unsqueeze_no_match_different_axes(self):
        model = oh.make_model(
            oh.make_graph(
                [
                    oh.make_node("Unsqueeze", ["X", "axes0"], ["xu"]),
                    oh.make_node("Unsqueeze", ["Y", "axes1"], ["yu"]),
                    oh.make_node("Mul", ["xu", "yu"], ["Z"]),
                ],
                "test",
                [
                ...
                [oh.make_tensor_value_info("Z", TensorProto.FLOAT, [3, 3])],
                [
                    onh.from_array(np.array([0], dtype=np.int64), name="axes0"),
                    onh.from_array(np.array([1], dtype=np.int64), name="axes1"),
                ],
            ),
            opset_imports=[oh.make_operatorsetid("", 18)],
        ...
        self.assertEqual(
            ["Unsqueeze", "Unsqueeze", "Mul"], [n.op_type for n in opt_onx.graph.node]
        )
        ref2 = ExtendedReferenceEvaluator(opt_onx)
        zz = ref2.run(None, feeds)[0]
        self.assertEqualArray(z, zz)
