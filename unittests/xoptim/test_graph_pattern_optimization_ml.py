...
class TestGraphPatternOptimizationMl(ExtTestCase):
    ...
    def test_tree_ensemble_regressor_concat_axis1(self):
        rule = "BRANCH_LEQ"
        ...
        graph = oh.make_graph(
            [node1, node2, oh.make_node("Concat", ["Ym1", "Ym2"], ["Y"], axis=1)],
            "ml",
            [oh.make_tensor_value_info("X", TensorProto.FLOAT, ["batch", 3])],
            [oh.make_tensor_value_info("Y", TensorProto.FLOAT, ["batch", 2])],
        )
        onx = oh.make_model_gen_version(
            graph,
            opset_imports=[oh.make_opsetid("", TARGET_OPSET), oh.make_opsetid("ai.onnx.ml", 4)],
    ...
    def test_tree_ensemble_regressor_concat_axis1_sigmoid(self):
        rule = "BRANCH_LEQ"
        ...
        opt_onx = gr.to_onnx(optimize=True)
        self.assertEqual(
            ["TreeEnsembleRegressor", "Sigmoid"], [n.op_type for n in opt_onx.graph.node]
        )
    ...
    def test_tree_ensemble_regressor_concat_axis0(self):
        rule = "BRANCH_LEQ"
        ...
        graph = oh.make_graph(
            [node1, node2, oh.make_node("Concat", ["Ym1", "Ym2"], ["Y"], axis=0)],
            "ml",
            [oh.make_tensor_value_info("X", TensorProto.FLOAT, ["batch", 3])],
            [oh.make_tensor_value_info("Y", TensorProto.FLOAT, ["batch2", 1])],
        )
        onx = oh.make_model_gen_version(
            graph,
            opset_imports=[oh.make_opsetid("", TARGET_OPSET), oh.make_opsetid("ai.onnx.ml", 4)],
        ...
        opt_onx = gr.to_onnx(optimize=True)
        self.assertEqual(
            ["TreeEnsembleRegressor", "Transpose", "Reshape"],
        ...
        ref = ExtendedReferenceEvaluator(onx)
        expected = ref.run(None, feeds)[0]
        self.assertEqual(expected.shape, (feeds["X"].shape[0] * 2, 1))
    ...
    def test_tree_ensemble_regressor_concat_axis0_sigmoid(self):
        rule = "BRANCH_LEQ"
        ...
        graph = oh.make_graph(
            [
                node1,
                node2,
                oh.make_node("Sigmoid", ["Ym1"], ["Ys1"], name="Sig1"),
                oh.make_node("Sigmoid", ["Ym2"], ["Ys2"], name="Sig2"),
                oh.make_node("Concat", ["Ys1", "Ys2"], ["Y"], axis=0),
            ],
            "ml",
            [oh.make_tensor_value_info("X", TensorProto.FLOAT, ["batch", 3])],
            [oh.make_tensor_value_info("Y", TensorProto.FLOAT, ["batch2", 1])],
        ...
        opt_onx = gr.to_onnx(optimize=True)
        self.assertEqual(
            ["TreeEnsembleRegressor", "Sigmoid", "Transpose", "Reshape"],
            [n.op_type for n in opt_onx.graph.node],
        )
    ...
    def test_tree_ensemble_regressor_concat_axis1_base_values(self):
        rule = "BRANCH_LEQ"
    ...
    def test_tree_ensemble_regressor_concat_axis1_base_values_opset2(self):
        rule = "BRANCH_LEQ"
