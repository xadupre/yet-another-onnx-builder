...
class ShapeBuilderCostRep(onnx.backend.base.BackendRep):
    ...
    def run(self, inputs, **kwargs):
        ...
        cost = shape_builder.run_model(self._dyn_model, exc=True, inference=InferenceMode.COST)
        assert len(cost) == len(
            self._dyn_model.graph.node
        ), f"Mismatch {len(cost)=}, {len(self._dyn_model.graph.node)=}"
        ...
        # dynamic
        shape_builder = BasicShapeBuilder(verbose=int(os.environ.get("VERBOSE", "0")))
        cost = shape_builder.run_model(self._model, exc=True, inference=InferenceMode.COST)
        assert len(cost) == len(
            self._model.graph.node
        ), f"Mismatch {len(cost)=}, {len(self._model.graph.node)=}"
        assert all(isinstance(t, tuple) for t in cost), "One cost is missing."
        assert all(len(t) == 3 for t in cost), "One cost is malformed."
        try:
...
class ShapeBuilderCost(onnx.backend.base.Backend):
    @classmethod
    ...
    def prepare(
        ...
        return ShapeBuilderCostRep(model)


backend_test = onnx.backend.test.BackendTest(ShapeBuilderCost())
