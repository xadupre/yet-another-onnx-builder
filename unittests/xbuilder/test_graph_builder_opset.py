...
class TestOpsetAbs(ExtTestCase):
    def test_abs(self):
        gr = _builder()
        gr.make_tensor_input("X", TFLOAT, ("a",), is_dimension=False)
...
class TestOpsetAdd(ExtTestCase):
    def test_add(self):
        gr = _builder()
        gr.make_tensor_input("X", TFLOAT, ("a",), is_dimension=False)
        gr.make_tensor_input("Y", TFLOAT, ("a",), is_dimension=False)
...
class TestOpsetAnd(ExtTestCase):
    def test_and(self):
        gr = _builder()
        gr.make_tensor_input("X", TBOOL, ("a",), is_dimension=False)
        gr.make_tensor_input("Y", TBOOL, ("a",), is_dimension=False)
        out = gr.op.And("X", "Y", outputs=["Z"])
        gr.make_tensor_output(out, TBOOL, ("a",), indexed=False, is_dimension=False)
        onx = gr.to_onnx()
...
class TestOpsetArgMax(ExtTestCase):
    def test_argmax(self):
        ...
        gr.make_tensor_output(out, TINT64, ("a",), indexed=False, is_dimension=False)
        onx = gr.to_onnx()
...
class TestOpsetCastLike(ExtTestCase):
    def test_castlike(self):
        gr = _builder()
        gr.make_tensor_input("X", TFLOAT, ("a",), is_dimension=False)
        gr.make_tensor_input("target_type", TINT64, ("a",), is_dimension=False)
...
class TestOpsetCompress(ExtTestCase):
    def test_compress(self):
        gr = _builder()
        gr.make_tensor_input("X", TFLOAT, ("a",), is_dimension=False)
        gr.make_tensor_input("condition", TBOOL, ("a",), is_dimension=False)
        out = gr.op.Compress("X", "condition", outputs=["Y"])
        gr.make_tensor_output(out, TFLOAT, (None,), indexed=False, is_dimension=False)
        onx = gr.to_onnx()
...
class TestOpsetConcat(ExtTestCase):
    def test_concat(self):
        gr = _builder()
        gr.make_tensor_input("X", TFLOAT, ("a",), is_dimension=False)
        gr.make_tensor_input("Y", TFLOAT, ("b",), is_dimension=False)
...
class TestOpsetConstant(ExtTestCase):
    def test_constant(self):
        ...
        gr.make_tensor_output(out, TFLOAT, (), indexed=False, is_dimension=False)
        onx = gr.to_onnx()
        (result,) = _run(onx, {})
...
class TestOpsetConstantOfShape(ExtTestCase):
    def test_constant_of_shape(self):
        gr = _builder()
        gr.make_tensor_input("shape", TINT64, ("n",), is_dimension=False)
...
class TestOpsetDropout(ExtTestCase):
    def test_dropout(self):
        ...
        gr.make_tensor_output(out, TFLOAT, ("a",), indexed=False, is_dimension=False)
        gr.make_tensor_output(mask, TBOOL, ("a",), indexed=False, is_dimension=False)
        onx = gr.to_onnx()
...
class TestOpsetEqual(ExtTestCase):
    def test_equal(self):
        gr = _builder()
        gr.make_tensor_input("X", TINT64, ("a",), is_dimension=False)
        gr.make_tensor_input("Y", TINT64, ("a",), is_dimension=False)
...
class TestOpsetExpand(ExtTestCase):
    def test_expand(self):
        ...
        gr.make_tensor_output(out, TFLOAT, ("c", "d"), indexed=False, is_dimension=False)
        onx = gr.to_onnx()
        x = np.array([[1.0, 2.0, 3.0, 4.0]], dtype=np.float32)
        (result,) = _run(onx, {"X": x})
...
class TestOpsetFlatten(ExtTestCase):
    def test_flatten(self):
        ...
        gr.make_tensor_output(out, TFLOAT, ("a", None), indexed=False, is_dimension=False)
        onx = gr.to_onnx()
...
class TestOpsetGather(ExtTestCase):
    def test_gather(self):
        ...
        gr.make_tensor_output(out, TFLOAT, ("c", "b"), indexed=False, is_dimension=False)
        onx = gr.to_onnx()
...
class TestOpsetGatherElements(ExtTestCase):
    def test_gather_elements(self):
        gr = _builder()
        gr.make_tensor_input("X", TFLOAT, ("a", "b"), is_dimension=False)
        gr.make_tensor_input("indices", TINT64, ("a", "b"), is_dimension=False)
...
class TestOpsetGemm(ExtTestCase):
    def test_gemm(self):
        gr = _builder()
        gr.make_tensor_input("A", TFLOAT, ("m", "k"), is_dimension=False)
        gr.make_tensor_input("B", TFLOAT, ("k", "n"), is_dimension=False)
        out = gr.op.Gemm("A", "B", outputs=["Y"])
        gr.make_tensor_output(out, TFLOAT, ("m", "n"), indexed=False, is_dimension=False)
        onx = gr.to_onnx()
...
class TestOpsetRange(ExtTestCase):
    def test_range(self):
        ...
        gr.make_tensor_output(out, TINT64, (None,), indexed=False, is_dimension=False)
        onx = gr.to_onnx()
        (result,) = _run(onx, {})
...
class TestOpsetScatterElements(ExtTestCase):
    def test_scatter_elements(self):
        ...
        gr.make_tensor_input("indices", TINT64, ("a", "b"), is_dimension=False)
        gr.make_tensor_input("updates", TFLOAT, ("a", "b"), is_dimension=False)
...
class TestOpsetSqueeze(ExtTestCase):
    def test_squeeze(self):
        gr = _builder()
        gr.make_tensor_input("X", TFLOAT, ("a", "b", "c"), is_dimension=False)
        axes = gr.make_initializer("", np.array([1], dtype=np.int64))
        out = gr.op.Squeeze("X", axes, outputs=["Y"])
        gr.make_tensor_output(out, TFLOAT, ("a", "c"), indexed=False, is_dimension=False)
        onx = gr.to_onnx()
...
class TestOpsetTopK(ExtTestCase):
    def test_topk(self):
        ...
        gr.make_tensor_output(values, TFLOAT, (None,), indexed=False, is_dimension=False)
        gr.make_tensor_output(indices, TINT64, (None,), indexed=False, is_dimension=False)
        onx = gr.to_onnx()
...
class TestOpsetTranspose(ExtTestCase):
    def test_transpose(self):
        ...
        gr.make_tensor_output(out, TFLOAT, ("b", "a"), indexed=False, is_dimension=False)
        onx = gr.to_onnx()
        x = np.arange(6, dtype=np.float32).reshape(2, 3)
        (result,) = _run(onx, {"X": x})
...
class TestOpsetUnsqueeze(ExtTestCase):
    def test_unsqueeze(self):
        ...
        gr.make_tensor_output(out, TFLOAT, ("c", "a"), indexed=False, is_dimension=False)
        onx = gr.to_onnx()
        x = np.array([1.0, 2.0, 3.0], dtype=np.float32)
        (result,) = _run(onx, {"X": x})
...
class TestSqueezeAnyOpset(ExtTestCase):
    ...
    def test_squeeze_any_opset_one_input(self):
        gr = _builder()
        gr.make_tensor_input("X", TFLOAT, (None, None, None), is_dimension=False)
