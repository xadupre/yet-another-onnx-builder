...
class TestXGBoostRFRegressor(ExtTestCase):
    ...
    def test_xgbrf_regressor_float64(self):
        """XGBRFRegressor with float64 input."""
        from xgboost import XGBRFRegressor
        ...
        expected = reg.predict(X32).astype(np.float32).astype(np.float64).reshape(-1, 1)
        self.assertEqualArray(expected, predictions, atol=1e-4)
    ...
    def test_xgbrf_regressor_dtypes_opsets(self):
        """XGBRFRegressor: float32/float64 x ai.onnx.ml opset 3 and 5."""
        from xgboost import XGBRFRegressor
    ...
    def test_xgbrf_regressor_pipeline(self):
