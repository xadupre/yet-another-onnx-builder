"""
Unit tests for yobx.sklearn.covariance.OAS converter.
"""
...
class TestOAS(ExtTestCase):
    ...
    def test_oas_float32(self):
        ...
        expected = oas.mahalanobis(X).astype(np.float32)
    ...
    def test_oas_pipeline(self):
        ...
        pipe = Pipeline([("scaler", StandardScaler()), ("oas", OAS())])
        pipe.fit(X)
        onx = to_onnx(pipe, (X,))
        ...
        oas = pipe.named_steps["oas"]
        scaler = pipe.named_steps["scaler"]
        X_scaled = scaler.transform(X).astype(np.float32)
        expected = oas.mahalanobis(X_scaled).astype(np.float32)
        self.assertEqualArray(expected, mahal_onnx, atol=1e-5)
