"""
Unit tests for yobx.sklearn.preprocessing.MaxAbsScaler converter.
"""
...
class TestMaxAbsScaler(ExtTestCase):
    def test_max_abs_scaler(self):
        ...
        expected = mas.transform(X).astype(np.float32)
    ...
    def test_max_abs_scaler_float64(self):
        from sklearn.preprocessing import MaxAbsScaler
        from yobx.sklearn import to_onnx
        ...
        ref = ExtendedReferenceEvaluator(onx)
        result = ref.run(None, {"X": X})[0]
        expected = mas.transform(X).astype(np.float64)
        self.assertEqualArray(expected, result, atol=1e-10)

    def test_pipeline_max_abs_scaler_logistic_regression(self):
        from sklearn.preprocessing import MaxAbsScaler
        ...
        X = np.array([[1, 2], [3, 4], [5, 6], [7, 8]], dtype=np.float32)
        y = np.array([0, 0, 1, 1])
        pipe = Pipeline([("scaler", MaxAbsScaler()), ("clf", LogisticRegression())])
        pipe.fit(X, y)
