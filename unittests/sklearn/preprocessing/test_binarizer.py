"""
Unit tests for yobx.sklearn.preprocessing.Binarizer converter.
"""

import unittest
import numpy as np
from yobx.ext_test_case import ExtTestCase, requires_sklearn
from yobx.reference import ExtendedReferenceEvaluator


@requires_sklearn("1.4")
class TestBinarizer(ExtTestCase):
    def test_binarizer_default_threshold(self):
        from sklearn.preprocessing import Binarizer
        from yobx.sklearn import to_onnx

        X = np.array([[-1.0, 0.0, 0.5, 1.0, 2.0]], dtype=np.float32)
        binarizer = Binarizer()

        onx = to_onnx(binarizer, (X,))

        # Check graph structure: should use Greater + Cast
        op_types = [n.op_type for n in onx.proto.graph.node]
        self.assertIn("Greater", op_types)
        self.assertIn("Cast", op_types)

        ref = ExtendedReferenceEvaluator(onx)
        result = ref.run(None, {"X": X})[0]
        expected = binarizer.transform(X).astype(np.float32)
        self.assertEqualArray(expected, result, atol=1e-5)

        sess = self.check_ort(onx)
        ort_result = sess.run(None, {"X": X})[0]
        self.assertEqualArray(expected, ort_result, atol=1e-5)

    def test_binarizer_custom_threshold(self):
        from sklearn.preprocessing import Binarizer
        from yobx.sklearn import to_onnx

        X = np.array([[0.5, 1.0, 1.5, 2.0, 3.0]], dtype=np.float32)
        binarizer = Binarizer(threshold=1.0)

        onx = to_onnx(binarizer, (X,))

        ref = ExtendedReferenceEvaluator(onx)
        result = ref.run(None, {"X": X})[0]
        expected = binarizer.transform(X).astype(np.float32)
        self.assertEqualArray(expected, result, atol=1e-5)

        sess = self.check_ort(onx)
        ort_result = sess.run(None, {"X": X})[0]
        self.assertEqualArray(expected, ort_result, atol=1e-5)

    def test_binarizer_multirow(self):
        from sklearn.preprocessing import Binarizer
        from yobx.sklearn import to_onnx

        rng = np.random.default_rng(0)
        X = rng.standard_normal((20, 5)).astype(np.float32)
        binarizer = Binarizer(threshold=0.3)

        onx = to_onnx(binarizer, (X,))

        ref = ExtendedReferenceEvaluator(onx)
        result = ref.run(None, {"X": X})[0]
        expected = binarizer.transform(X).astype(np.float32)
        self.assertEqualArray(expected, result, atol=1e-5)

        sess = self.check_ort(onx)
        ort_result = sess.run(None, {"X": X})[0]
        self.assertEqualArray(expected, ort_result, atol=1e-5)

    def test_pipeline_binarizer_logistic_regression(self):
        from sklearn.preprocessing import Binarizer
        from sklearn.linear_model import LogisticRegression
        from sklearn.pipeline import Pipeline
        from yobx.sklearn import to_onnx

        rng = np.random.default_rng(42)
        X = rng.standard_normal((30, 4)).astype(np.float32)
        y = (X[:, 0] > 0).astype(int)

        pipe = Pipeline([("bin", Binarizer(threshold=0.0)), ("clf", LogisticRegression())])
        pipe.fit(X, y)

        onx = to_onnx(pipe, (X,))

        op_types = [n.op_type for n in onx.proto.graph.node]
        self.assertIn("Greater", op_types)
        self.assertIn("Cast", op_types)
        self.assertIn("Gemm", op_types)

        ref = ExtendedReferenceEvaluator(onx)
        results = ref.run(None, {"X": X})
        label, proba = results[0], results[1]

        expected_label = pipe.predict(X)
        expected_proba = pipe.predict_proba(X).astype(np.float32)
        self.assertEqualArray(expected_label, label)
        self.assertEqualArray(expected_proba, proba, atol=1e-5)

        sess = self.check_ort(onx)
        ort_results = sess.run(None, {"X": X})
        self.assertEqualArray(expected_label, ort_results[0])
        self.assertEqualArray(expected_proba, ort_results[1], atol=1e-5)


if __name__ == "__main__":
    unittest.main(verbosity=2)
