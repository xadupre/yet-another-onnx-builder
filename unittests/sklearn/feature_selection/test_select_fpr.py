"""
Unit tests for yobx.sklearn.feature_selection.SelectFpr converter.
"""
...
class TestSelectFpr(ExtTestCase):
    ...
    def test_select_fpr_float32(self):
        from sklearn.feature_selection import SelectFpr, f_classif
        from yobx.sklearn import to_onnx

        X, y = self._make_data(np.float32)
        sel = SelectFpr(score_func=f_classif, alpha=0.5)
        sel.fit(X, y)
    ...
    def test_select_fpr_float64(self):
        from sklearn.feature_selection import SelectFpr, f_classif
        from yobx.sklearn import to_onnx
    ...
    def test_select_fpr_pipeline_float32(self):
        from sklearn.feature_selection import SelectFpr, f_classif
        ...
        X, y = self._make_data(np.float32)
        pipe = Pipeline(
            [("sel", SelectFpr(score_func=f_classif, alpha=0.5)), ("clf", LogisticRegression())]
        )
        pipe.fit(X, y)
    ...
    def test_select_fpr_pipeline_float64(self):
        from sklearn.feature_selection import SelectFpr, f_classif
        from sklearn.linear_model import LogisticRegression
        from sklearn.pipeline import Pipeline
        from yobx.sklearn import to_onnx
