"""
Unit tests for yobx.sklearn.feature_selection RFE and RFECV converters.
"""
...
class TestSklearnRFE(ExtTestCase):
    ...
    def test_rfe_float32(self):
        from sklearn.feature_selection import RFE
        from sklearn.linear_model import LogisticRegression
        from yobx.sklearn import to_onnx

        X, y = self._make_data(np.float32)
        sel = RFE(estimator=LogisticRegression(), n_features_to_select=5)
        sel.fit(X, y)
    ...
    def test_rfe_pipeline_float32(self):
        from sklearn.feature_selection import RFE
        from sklearn.linear_model import LogisticRegression
    ...
    def test_rfe_pipeline_float64(self):
        from sklearn.feature_selection import RFE
        from sklearn.linear_model import LogisticRegression
        from sklearn.pipeline import Pipeline
        from yobx.sklearn import to_onnx
...
@requires_sklearn("1.4")
class TestSklearnRFECV(ExtTestCase):
    def _make_data(self, dtype, n_samples=50, n_features=10, seed=42):
        rng = np.random.default_rng(seed)
        X = rng.standard_normal((n_samples, n_features)).astype(dtype)
        y = (X[:, 0] > 0).astype(int)
    ...
    def test_rfecv_pipeline_float32(self):
        from sklearn.feature_selection import RFECV
        from sklearn.linear_model import LogisticRegression
        from sklearn.pipeline import Pipeline
        from yobx.sklearn import to_onnx
    ...
    def test_rfecv_pipeline_float64(self):
        from sklearn.feature_selection import RFECV
        from sklearn.linear_model import LogisticRegression
        from sklearn.pipeline import Pipeline
        from yobx.sklearn import to_onnx
