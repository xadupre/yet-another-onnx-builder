"""
Unit tests for yobx.sklearn.category_encoders.PolynomialEncoder converter.
"""
...
class TestPolynomialEncoder(ExtTestCase):
    ...
    def test_polynomial_encoder_float32(self):
        """Basic two-column encoding with float32 inputs."""
        from category_encoders import PolynomialEncoder

        X_df, dtype = self._make_data(np.float32)
        enc = PolynomialEncoder(cols=["cat1", "cat2"])
        enc.fit(X_df)
        self._run_test(enc, X_df[:8], dtype)

    def test_polynomial_encoder_float64(self):
        """Basic two-column encoding with float64 inputs."""
        from category_encoders import PolynomialEncoder

        X_df, dtype = self._make_data(np.float64)
        enc = PolynomialEncoder(cols=["cat1", "cat2"])
        enc.fit(X_df)
        self._run_test(enc, X_df[:8], dtype)

    def test_polynomial_encoder_single_col_float32(self):
        """Single categorical column with float32."""
        from category_encoders import PolynomialEncoder

        X_df, dtype = self._make_data(np.float32)
        enc = PolynomialEncoder(cols=["cat1"])
        enc.fit(X_df)
        self._run_test(enc, X_df[:8], dtype)

    def test_polynomial_encoder_single_col_float64(self):
    ...
    def test_polynomial_encoder_unknown_category(self):
        """Unknown categories map to zero contrast values."""
        from category_encoders import PolynomialEncoder
        from yobx.sklearn import to_onnx
    ...
    def test_polynomial_encoder_nan_input(self):
        """NaN inputs map to zero contrast values."""
        from category_encoders import PolynomialEncoder
        from yobx.sklearn import to_onnx
        ...
        X_test_df = pd.DataFrame(
            {
                "cat1": [0.0, 1.0, float("nan"), 2.0],
                "cat2": [0.0, 1.0, 0.0, 1.0],
                "num": [0.0, 1.0, 2.0, 3.0],
            }
        )
        X_np = X_test_df.values.astype(dtype)
    ...
    def test_polynomial_encoder_all_numeric_passthrough(self):
    ...
    def test_polynomial_encoder_pipeline(self):
        """PolynomialEncoder in a sklearn Pipeline."""
        from category_encoders import PolynomialEncoder
        from sklearn.linear_model import LinearRegression
        from sklearn.pipeline import Pipeline
        from yobx.sklearn import to_onnx

        X_df, _dtype = self._make_data(np.float32)
        y = np.arange(len(X_df), dtype=np.float32)
        pipe = Pipeline(
            [("enc", PolynomialEncoder(cols=["cat1", "cat2"])), ("reg", LinearRegression())]
        )
        pipe.fit(X_df, y)
    ...
    def test_string_basic(self):
        """String categorical columns with PolynomialEncoder."""
        from category_encoders import PolynomialEncoder
        from yobx.sklearn import to_onnx
