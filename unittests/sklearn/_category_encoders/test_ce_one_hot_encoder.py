"""
Unit tests for yobx.sklearn.category_encoders.OneHotEncoder converter.
"""
...
class TestCEOneHotEncoder(ExtTestCase):
    ...
    def test_unknown_category_value_default(self):
        """Unknown categories produce all-zero indicator block (default handle_unknown)."""
        from category_encoders import OneHotEncoder
        from yobx.sklearn import to_onnx
    ...
    def test_unknown_category_return_nan(self):
        ...
        X_test = pd.DataFrame({"cat": [0.0, 99.0, np.nan], "num": [1.0, 2.0, 3.0]})
        X_np = X_test.values.astype(np.float32)
    ...
    def test_nan_input_produces_zeros(self):
        ...
        X_test = pd.DataFrame({"cat": [0.0, np.nan, 1.0], "num": [1.0, 2.0, 3.0]})
        X_np = X_test.values.astype(np.float32)
    ...
    def test_pipeline(self):
        """OneHotEncoder (category_encoders) in a sklearn Pipeline."""
        from category_encoders import OneHotEncoder
    ...
    def test_string_single_column(self):
        """Single string categorical column with category_encoders OneHotEncoder."""
        from category_encoders import OneHotEncoder
        from yobx.sklearn import to_onnx
