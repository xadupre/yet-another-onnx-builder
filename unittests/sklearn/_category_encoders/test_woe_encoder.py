"""
Unit tests for yobx.sklearn.category_encoders.WOEEncoder converter.
"""
...
class TestWOEEncoder(ExtTestCase):
    def _make_data(self, dtype=np.float32):
        ...
        # Binary target required by WOEEncoder
    ...
    def test_woe_encoder_single_col_float32(self):
        """Single categorical column with float32."""
        from category_encoders import WOEEncoder

        X_df, y, dtype = self._make_data(np.float32)
        enc = WOEEncoder(cols=["cat1"])
        enc.fit(X_df, y)
        self._run_test(enc, X_df[:8], dtype)

    def test_woe_encoder_single_col_float64(self):
    ...
    def test_woe_encoder_unknown_category(self):
        """Unknown categories map to the handle_unknown WOE value."""
        from category_encoders import WOEEncoder
        from yobx.sklearn import to_onnx
    ...
    def test_woe_encoder_all_numeric_passthrough(self):
    ...
    def test_woe_encoder_pipeline(self):
        """WOEEncoder in a sklearn Pipeline."""
        from category_encoders import WOEEncoder
    ...
    def test_string_basic(self):
        """String categorical columns with WOEEncoder."""
        from category_encoders import WOEEncoder
        from yobx.sklearn import to_onnx
