"""
Unit tests for yobx.sklearn.category_encoders.TargetEncoder converter.
"""
...
class TestTargetEncoder(ExtTestCase):
    def _make_data(self, dtype=np.float32):
        ...
        y = np.tile([1, 0, 1, 0, 1, 0], 5).astype(float)
        return X, y, dtype
    ...
    def test_target_encoder_float32(self):
    ...
    def test_target_encoder_float64(self):
    ...
    def test_target_encoder_single_col_float32(self):
        """Single categorical column with float32."""
        from category_encoders import TargetEncoder

        X_df, y, dtype = self._make_data(np.float32)
        enc = TargetEncoder(cols=["cat1"])
        enc.fit(X_df, y)
        self._run_test(enc, X_df[:8], dtype)

    def test_target_encoder_single_col_float64(self):
    ...
    def test_target_encoder_unknown_category(self):
        """Unknown categories map to the handle_unknown target value."""
        from category_encoders import TargetEncoder
        from yobx.sklearn import to_onnx
    ...
    def test_target_encoder_all_numeric_passthrough(self):
        ...
        y = (X_df["a"] > 0).astype(float).values
    ...
    def test_target_encoder_pipeline(self):
        """TargetEncoder in a sklearn Pipeline."""
        from category_encoders import TargetEncoder
        ...
        pipe = Pipeline(
            [("enc", TargetEncoder(cols=["cat1", "cat2"])), ("reg", LinearRegression())]
        )
        pipe.fit(X_df, y_f)
