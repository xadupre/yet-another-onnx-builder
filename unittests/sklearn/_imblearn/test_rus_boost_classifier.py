"""
Unit tests for the RUSBoostClassifier converter.
"""
...
class TestSklearnRUSBoostClassifier(ExtTestCase):
    _X = np.array(
    ...
    def _check_clf(self, X, y, atol=1e-5, **kwargs):
        for dtype in (np.float32, np.float64):
            Xd = X.astype(dtype)
            clf = RUSBoostClassifier(random_state=0, **kwargs)
            clf.fit(Xd, y)
            onx = to_onnx(clf, (Xd,))
    ...
    def test_classifier_binary(self):
        """RUSBoostClassifier binary classification."""
        self._check_clf(self._X, self._y_bin)

    def test_classifier_multiclass(self):
        """RUSBoostClassifier multiclass classification."""
        self._check_clf(self._X, self._y_multi, n_estimators=5)

    def test_classifier_fewer_estimators(self):
        """RUSBoostClassifier with a smaller number of estimators."""
        self._check_clf(self._X, self._y_bin, n_estimators=3)

    def test_classifier_custom_base_estimator(self):
        """RUSBoostClassifier with a custom DecisionTreeClassifier base estimator."""
        for dtype in (np.float32, np.float64):
    ...
    def test_classifier_in_pipeline(self):
        """RUSBoostClassifier as last step in a sklearn Pipeline."""
        Xd = self._X.astype(np.float32)
        y = self._y_bin
        clf = RUSBoostClassifier(n_estimators=3, random_state=0)
