"""
Unit tests for the BalancedBaggingClassifier converter.
"""
...
class TestSklearnBalancedBaggingClassifier(ExtTestCase):
    _X = np.array(
    ...
    def _check_clf(self, X, y, atol=1e-5, **kwargs):
        for dtype in (np.float32, np.float64):
            Xd = X.astype(dtype)
            clf = BalancedBaggingClassifier(random_state=0, **kwargs)
            clf.fit(Xd, y)
            onx = to_onnx(clf, (Xd,))
    ...
    def test_classifier_in_pipeline(self):
        """BalancedBaggingClassifier as last step in a sklearn Pipeline."""
        Xd = self._X.astype(np.float32)
        y = self._y_bin
        clf = BalancedBaggingClassifier(n_estimators=3, random_state=0)
