...
def to_onnx(
    ...
    * **SQL / DataFrame / NumPy callable** — when *model* is a plain
    ...
    :param dynamic_shapes: optional specification of dynamic (symbolic)
        dimensions.  The exact format depends on the backend: a dict mapping
        axis indices to dim-name strings for sklearn / tensorflow / litert,
        or a dict / tuple following :func:`torch.export.export` conventions
        for torch.  Supported by all backends.
    ...
    :param kwargs: additional backend-specific keyword arguments forwarded
        verbatim to the selected converter.  Consult the documentation of
        each backend for the full list of accepted parameters.
