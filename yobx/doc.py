...
def plot_mermaid(
    ...
    When the ``mermaid-py`` library and the ``mermaid.ink`` online service are
    available the diagram is rendered to PNG and embedded in the axis.
    If rendering fails (e.g. no network access), the raw Mermaid source is
    displayed as plain text so the example still runs offline.
    ...
    exc = None
    try:
        from mermaid import Mermaid, MermaidError as MermaidApiError
    except ImportError as e:
        exc = e
        MermaidApiError = None
    ...
    if exc is None:
        try:
            ...
            renderer = Mermaid(smmd)
            img = Image.open(io.BytesIO(renderer.img_response.content))
            ax.imshow(np.asarray(img))
        except (MermaidApiError, requests.exceptions.RequestException, OSError) as e:
            exc = e
