...
if hasattr(transformers.cache_utils, "SlidingWindowCache"):
    ...
    def make_sliding_window_cache(
        ...
        """
        Creates an instance of :class:`transformers.cache_utils.SlidingWindowCache`.

        :param key_value_pairs: list of pairs of (key, values)
        :param cls_layers: unused
        :param cls_kwargs: unused
        :return: :class:`transformers.cache_utils.SlidingWindowCache`
        """
        key_value_pairs = _preprocess_key_value_pairs(key_value_pairs)
        ...
        if hasattr(cache, "layers") and len(ca.key_cache) == 0:
            ...
            cache_position = torch.arange(
                key_value_pairs[0][0].shape[2], dtype=torch.int64
            )
            for i, (key, value) in enumerate(key_value_pairs):
        ...
        if hasattr(cache, "layers") and len(key_value_pairs) < len(cache.layers):
            # The cache constructor may append extra empty layers; remove them.
            cache.layers[:] = cache.layers[-len(key_value_pairs) :]
        assert not hasattr(cache, "layers") or len(key_value_pairs) == len(cache.layers), (
            f"Unexpected number of layers in the cache ({len(cache.layers)}), "
            f"{len(key_value_pairs)} expected."
...
if hasattr(transformers.cache_utils, "HybridCache"):
    ...
    def make_hybrid_cache(
        ...
        :param key_value_pairs: list of pairs of (key, values)
        :param cls_layers: optional list of layer types (e.g. StaticLayer,
            DynamicSlidingWindowLayer) to recreate the proper layer types
        :param cls_kwargs: optional list of kwargs for each layer constructor
        :return: :class:`transformers.cache_utils.HybridCache`
        """
        key_value_pairs = _preprocess_key_value_pairs(key_value_pairs)
        is_sliding = [False] * len(key_value_pairs)
        if cls_layers:
            is_sliding = [
                getattr(cl, "__name__", "") in ("DynamicSlidingWindowLayer", "StaticSlidingWindowLayer")
        ...
        class _config:
            def __init__(self):
                self.head_dim = key_value_pairs[0][0].shape[-1]
                self.num_attention_heads = key_value_pairs[0][0].shape[1]
                self.num_key_value_heads = key_value_pairs[0][0].shape[1]
                self.num_hidden_layers = len(key_value_pairs)
                self.sliding_window = key_value_pairs[0][0].shape[2]
                self.max_position_embeddings = key_value_pairs[0][0].shape[2]
                self.layer_types = [
                    "sliding_attention" if sl else "full_attention" for sl in is_sliding
                ]
