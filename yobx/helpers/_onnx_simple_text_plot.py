...
def _append_succ_pred(
    ...
    if node.op_type in {"If", "Scan", "Loop", "Expression"}:
        for att in node.attribute:
            if att.type != onnx.AttributeProto.GRAPH or not hasattr(att, "g") or att.g is None:
                continue
...
def _get_hidden_inputs(nodes):
    ...
    for node in nodes:
        ...
        for att in node.attribute:
            if att.type != onnx.AttributeProto.GRAPH or not hasattr(att, "g") or att.g is None:
                continue
            hidden = _get_hidden_inputs(att.g.node)
...
def reorder_nodes_for_display(nodes, verbose=False):
    ...
    local_variables = _get_hidden_inputs(nodes)
    ...
    while len(done) < len(nodes):
        ...
        if best is None:
            raise RuntimeError(f"Wrong implementation (len(sequence)={len(sequences)}).")
        if verbose:
            print(
...
def onnx_simple_text_plot(
            ...
            for att in node.attribute:
                ...
                if att.name in att_display:
                    if att.type == onnx.AttributeProto.INT:
                        ...
                        atts.append("%s=%s" % (att.name, str(list(att.ints)).replace(" ", "")))
                    else:
                        done = False
                elif (
                    att.type == onnx.AttributeProto.GRAPH
                    and hasattr(att, "g")
                ...
                elif att.type == onnx.AttributeProto.INT:
                    val = str(att.i)
                elif att.type == onnx.AttributeProto.FLOAT:
                    val = str(att.f)
                elif att.type == onnx.AttributeProto.INTS:
    ...
    # inputs
    assert hasattr(model, "input"), f"type checking {type(model)}"
    line_name_new = {}
    ...
    for inp in model.input:
        if isinstance(inp, str):
            ...
            rows.append(
                "input: name=%r type=%r shape=%r" % (inp.name, _get_type(inp), _get_shape(inp))
            )
    ...
    for out in model.output:
        if isinstance(out, str):
            ...
            rows.append(
                "output: name=%r type=%r shape=%r" % (out.name, _get_type(out), _get_shape(out))
            )
