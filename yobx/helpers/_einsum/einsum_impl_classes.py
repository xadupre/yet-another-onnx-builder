...
class EinsumSubOp:
    ...
    def __init__(
        ...
        assert len(inputs) in (1, 2), f"Inputs must contains 1 or 2 inputs not {len(inputs)}."
        if name == "matmul" and len(inputs) != 2:
            raise RuntimeError(
                "Inputs must contains 2 inputs not %d for operator 'matmul'." % len(inputs)
            )
        for i, inp in enumerate(inputs):
            assert isinstance(
                inp, (int, EinsumSubOp)
            ), "Input %d has type %r, int or EinsumSubOp is expected." % (  # noqa: ISC001
                i,
                type(inp),
            )
        self._check_()
    ...
    def _check_arg_(self, name: str, typ: type, empty: bool = False):
        assert name in self.kwargs, f"Parameter {name!r} not found for operator {self.name!r}."
    ...
    def _compute_output_row_id(
        self, row: numpy.ndarray, row2: numpy.ndarray, ab: bool = False, verbose: bool = False
    ):
    ...
    def _compute_output_row_diagonal(
        ...
        for r in to_remove:
            for i in range(len(row)):
                assert (
                    row[i] != r
                ), "Unexpected result r=%r row=%r to_remove=%r diag=%r." % (  # noqa: ISC001
                    r,
                    row,
                    to_remove,
                    self.kwargs["diag"],
    ...
    def compute_output_row(
        ...
        if meth is None:
            raise NotImplementedError(f"compute_output_row not implemented for {self.name!r}.")
        if verbose and ab:
    ...
    def add_info(self, **kwargs: Dict[str, Any]):
        ...
        for k, v in kwargs.items():
            assert k not in self._info, f"Key {k!r} already added (operator {self.name!r})."
            self._info[k] = v
    ...
    def _check_shape_(self, m: numpy.ndarray):
        ...
        ), "Number of dimensions %r is different from expected value " "%d." % (  # noqa: ISC001
            m.shape,
            self.full_dim,
        )
    ...
    def _apply_matmul(
        ...
        if impl == "pyf":
            output = numpy_extended_dot_matrix(m1, m2, axes, left, right, verbose=verbose)
        elif impl == "py":
            output = numpy_extended_dot_python(m1, m2, axes, left, right, verbose=verbose)
        elif impl is None:
    ...
    def _apply_batch_dot(
        ...
        if verbose:
            print(
                ...
                "sum_axes=%r" % (self.name, m1.shape, m2.shape, batch_axes, keep_axes, sum_axes)
            )
        ...
        dimb = int(-1 if keep_axes is None else numpy.prod([m1.shape[i] for i in keep_axes]))
        ...
        if batch_kind in ("11", "N1", "N1"):
            ...
            if verbose:
                print("- %s, use gemm with shape %r, %r" % (self.name, m1sh.shape, m2sh.shape))
        ...
        if verbose:
            ...
            print("- %s, shapes=%r @ %r -> %r" % (self.name, m1sh.shape, m2sh.shape, dot.shape))
            print(
    ...
    def apply(self, data: Dict[int, Any], verbose: bool = False, **kwargs: Dict[str, Any]) -> Any:
        ...
        * 'matmul_impl': if None calls :func:`numpy.einsum` through
        ...
        if verbose:
            print()
            print("apply %r  (%s)." % (self.name, ", ".join([str(id(s)) for s in self.inputs])))
    ...
    def _to_onnx_batch_dot(
        ...
        **kwargs: Dict[str, Any],
        ...
        if len(batch_axes) > 1:
            ...
            yield helper.make_node("Gather", [name_shape1, name_batch_axes], [name_dim0g])
            yield helper.make_node("Gather", [name_shape2, name_batch_axes], [name_dim0bg])
        ...
        if keep_axes in (-1, None) or len(keep_axes) == 0:
            ...
            yield helper.make_node("Gather", [name_shape1, name_keep_axes], [name_dimbg])
        else:
            name_keep_axes = root + "_keep_axes"
            name_dimb = root + "_dimb"
        ...
        if len(left_set) > 0:
            ...
            yield helper.make_node("Gather", [name_shape1, name_left_set], [name_left_dim])
        ...
        if len(right_set) > 0:
            ...
            yield helper.make_node("Gather", [name_shape2, name_right_set], [name_right_dim])
        ...
        if diff > 0:
            ...
            yield numpy_helper.from_array(
                numpy.array([1 for i in range(diff)], dtype=numpy.int64), name=names_ones
            )
        ...
        **kwargs: Dict[str, Any],
        ...
        for node in meth(names, verbose=verbose, opset=opset, **kwargs):
            if hasattr(node, "output"):
                ...
                if verbose:
                    print("+ OP %r -- (%s - %d)" % (node.output[0], self.name, id(self)))
            elif verbose:
                # Initializer
...
class GraphEinsumSubOp:
    ...
    def simplify_mm_nodes(self, verbose: bool = False):
        ...
        for op in self:
            ...
            if op.name.endswith("_mm"):
                ...
                assert len(op.inputs) == 2, "Expecting 2 inputs for node %r not %r id=%r." % (
                    op.name,
                    len(op.inputs),
                    id(op),
                )
    ...
    def _replace_node_sequence(self, added: Optional[EinsumSubOp], deleted: List[EinsumSubOp]):
        """
        ...
        assert (
            key in forward
        ), "Key {} missing in all forward nodes (other keys {}), all keys:\n{}".format(
        ...
        for d in deleted:
            ...
            if id(d) in self._mark:
                ...
                assert (
                    len(dels) == 1
                ), "Input %d has more than one marked operator " "(%r)." % (  # noqa: ISC001
                    id(d),
                    dels,
                )
        ...
        while modif > 0:
            ...
            if len(candidates) > 0:
                ...
                assert len(perm1) == len(
                    perm2
                ), "Transposition should have the same length " "%r, %r." % (  # noqa: ISC001
                    perm1,
                    perm2,
                )
                perm = list(perm1)
