...
def _numpy_extended_dot_equation(
    m1_dim: int, m2_dim: int, axes: Tuple[int, ...], left: Tuple[int, ...], right: Tuple[int, ...]
) -> str:
    """
    ...
    assert (
        m1_dim == m2_dim
    ), "Matrices m1 and m2 must have the same number of dimensions, m1=%r, m2=%r." % (
        m1_dim,
        m2_dim,
    )
...
def numpy_extended_dot_output_shape(
    ...
    for i in right:
        if i in left and m1.shape[i] != m2.shape[i] and m1.shape[i] != 1 and m2.shape[i] != 1:
            raise RuntimeError(
...
def _numpy_extended_dot_python_intermediate(
...
) -> Tuple[List[str], numpy.ndarray, Dict[str, int], List[bool], List[bool], numpy.ndarray]:
    ...
    broadcast = [common[i] and m1_shape[pos[i]] != m2_shape[pos[i]] for i in range(len(common))]
...
def _numpy_extended_dot_python_update_broadcast(
    ...
    if verbose:
        print(
            ...
            % (
                ...
                _numpy_extended_dot_equation(len(m1.shape), len(m1.shape), axes, left, right),
            )
        )
        print(
    ...
    for i in range(len(broadcast)):
        ...
        if verbose:
            print("[GENERICDOT] name=%s dim=%r let=%r inp=%r p=%r" % (names[i], dim, let, inp, p))
...
def numpy_extended_dot_python(
    ...
    if verbose:
        ...
        print("[GENERICDOT] shape1=%r shape2=%r shape=%r" % (m1.shape, m2.shape, res.shape))
...
def numpy_extended_dot_matrix(
    ...
    if len(set(axes) & set(left)) == 0 and len(set(axes) & set(right)) == 0:
        ...
        if right_no_left:
            ...
            if verbose:
                print(
                    "[GENERICDOT] reducesumL=%r, %r -> %r" % (right_no_left, m1.shape, red1.shape)
                )
        else:
            red1 = m1
        ...
        if left_no_right:
            ...
            if verbose:
                print(
                    "[GENERICDOT] reducesumR=%r, %r -> %r" % (left_no_right, m2.shape, red2.shape)
                )
        else:
            red2 = m2
        ...
        i_axes = [
            (-1 if i in common_axes else (1 if i in axes else 0), i) for i in range(len(m1.shape))
        ]
        i_axes.sort()
        ...
        assert len(current_shape) == len(
            final_shape
        ), "Shapes mismatch %r > %r, shape1=%r shape2=%r axes=%r left=%r right=%r." % (
            current_shape,
            final_shape,
            m1.shape,
            m2.shape,
        ...
        if r_axes and not l_axes:
            ...
            if verbose:
                ...
                eq2 = _numpy_extended_dot_equation(
                    len(m1.shape), len(m1.shape), tuple(new_axes), tuple(new_left), tuple(right)
                )
                print(
