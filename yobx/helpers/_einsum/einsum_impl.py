...
def _basic_verification(lengths: List[int], shapes: List[Tuple[int, ...]], equation: str):
    ...
    for i, (le, sh) in enumerate(zip(lengths, shapes)):
        assert le == len(
            sh
        ), "Inputs %d has %d dimensions but shapes %r has %d in equation %r." % (  # noqa: ISC001
            i,
            le,
            sh,
            len(sh),
...
def _apply_einsum_matmul(
    ...
    *op_matmul* (see :func:`decompose_einsum_equation`).
    ...
    assert op_matmul in allowed, f"Unknown operator op_matmul={op_matmul!r} not in {allowed!r}."
    if op_matmul == "matmul":
        if verbose:
            print(f"  -- MATMUL -> matmul axes={axes!r} left={left!r} right={right!r}")
        yield EinsumSubOp(fd, "matmul", op1, op2, axes=axes, left=left, right=right, ndim=ndim)
        ...
        if verbose:
            print("  -- MATMUL -> batch_dot axes=%r left=%r right=%r" % (axes, left, right))
        ...
        if right_no_left:
            ...
            op1 = EinsumSubOp(fd, "reduce_sum_mm", op1, op2, axes=tuple(sorted(right_no_left)))
            yield op1
        ...
        # Transpose
        i_axes = [(-1 if i in common_axes else (1 if i in axes else 0), i) for i in range(ndim)]
        i_axes.sort()
