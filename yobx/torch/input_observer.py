...
class InputObserverInfo:
    ...
    def infer_arguments(
        ...
        if index_or_candidate is None:
            ...
            torch._check(
                ...
                lambda: (
                    f"No stored input set for index={index_or_candidate}<{len(self.inputs)}."
                ),
            )
    ...
    def _resolve_dim_label(
        self, arg_or_kwarg: int | str, dim: int, dim_names: dict[str | int, dict[int, str]]
    ) -> str:
        ...
        *dim_names* provides an explicit label for this input and dimension it is
        ...
        Args:
            arg_or_kwarg: The positional index (``int``) or keyword name (``str``)
                that identifies which input this tensor belongs to, as stored in
    ...
    def _build_auto_dim_names(
        self, shape_lists: list[list[tuple[int, ...] | None]], dynamic_shapes: list[list[int]]
...
class InputObserver:
    ...
    def infer_dynamic_shapes(
        ...
        return self.info.infer_dynamic_shapes(
            set_batch_dimension_for=set_batch_dimension_for, dim_names=dim_names
        )
