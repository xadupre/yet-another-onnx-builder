...
# var/std still need a slightly wider tolerance than the global default because
# the reduction accumulates rounding error, but float32 uses exact arithmetic
# throughout so the gap is small.
ATOL_OPS_FLOAT32: Dict[str, float] = {
    ...
    "var": 1e-1,  # var = std²: float32 accumulation; slightly larger than std_atol
    "var_mean": 1e-1,  # same compound error as var
}
# Per-op absolute tolerance overrides for torch.float16.
# _std_var_correction upcasts float16→float32 for the reduction (matching
# PyTorch's internal behaviour), so var/var_mean errors are now std-level.
ATOL_OPS_FLOAT16: Dict[str, float] = {
    ...
    "var": 2e-1,  # upcast to float32 keeps error at std-level
    "var_mean": 2e-1,  # same compound error as var
}

# Per-op absolute tolerance overrides for torch.bfloat16.
# _std_var_correction upcasts bfloat16→float32 for the reduction (matching
# PyTorch's internal behaviour), so var/var_mean errors are now std-level.
ATOL_OPS_BFLOAT16: Dict[str, float] = {
    ...
    "var": 3e-1,  # upcast to float32 keeps error at std-level
