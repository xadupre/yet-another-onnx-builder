...
try:
    ...
    _OnnxExporterBase = None  # type: ignore[assignment,misc]


def _require_transformers() -> None:
    """Raises :class:`ImportError` when ``transformers`` is not installed."""
    if _OnnxExporterBase is None:
        raise ImportError(
...
if _OnnxExporterBase is not None:
    ...
    class YobxOnnxExporter(_OnnxExporterBase):  # type: ignore[valid-type]
        ...
        def export(
            ...
            if isinstance(config, dict):
                ...
                raise TypeError(
                    f"Expected config to be an OnnxConfig or dict, got {type(config)}"
                )
    ...
    class YobxOnnxExporter:  # type: ignore[no-redef]
        """
        Stub raised when ``transformers`` is not installed.

        Instantiating this class raises :class:`ImportError`.
        """
        ...
        def export(self, *args: Any, **kwargs: Any) -> Any:
            """Raises :class:`ImportError` — ``transformers`` is not installed."""
            _require_transformers()
