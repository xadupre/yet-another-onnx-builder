...
def _causal_attention_mask(
    ...
    if attn_mask is None:
        ...
        attn_mask = g.op.ConstantOfShape(
            size, value=onh.from_array(np.array([1], dtype=dtype)), name=name
        )
...
def aten_scaled_dot_product_attention(
    ...
    *S* is the source sequence length, and E is the embedding size.
    ...
    key_scaled = g.op.Mul(key, sc, name=name)
    key_transposed_scaled = g.op.Transpose(key_scaled, perm=key_transposed_axes, name=name)
