...
def convert_reshape(
    ...
    if not g.has_shape(outputs[0]):
        ...
        if tf_dims is not None and g.has_shape(op.inputs[0].name):
            ...
            # When the output rank equals the input rank, a None output dim at position idx
            # very likely corresponds to the same symbolic dim as input[idx], so we can reuse
            # that name.  When the ranks differ (e.g. rank-3 input reshaped to rank-2 output),
            # the mapping is ambiguous — a None output dim might represent a product of
            # multiple input dims — so we must NOT reuse the input symbolic name.  Using a
            # fresh "rd<idx>" name ensures ShapeBasedEditDistanceReshapePattern cannot anchor
            # on a shared symbolic dim and incorrectly collapse static input dims into a wrong
            # constant (e.g. turning (batch,4,4)→(batch*4,4) into a static Reshape [0,16]).
            rank_unchanged = len(tf_dims) == len(in_shape)

            def _map_dim(d: Any, idx: int) -> Any:
                if d is not None:
                    return d
                # Only reuse the input symbolic name when the rank is preserved (safe mapping).
                if rank_unchanged and idx < len(in_shape) and isinstance(in_shape[idx], str):
                    return in_shape[idx]
                # Use a fresh unique name so downstream optimisers cannot mis-identify
                # this dimension as a product of the static input dimensions.
                return f"rd{idx}"
...
def convert_transpose(
    ...
    When the permutation is a computed constant (e.g. the output of a
    ``ConcatV2`` of ``Const`` tensors as generated by ``tf.tensordot``), the
