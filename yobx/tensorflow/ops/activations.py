...
def convert_relu(g: GraphBuilderExtendedProtocol, sts: Dict[str, Any], outputs: List[str], op) -> str:
    """TF ``Relu`` → ONNX ``Relu``."""
    return g.op.Relu(op.inputs[0].name, outputs=outputs, name=op.name)


@register_tf_op_converter("Relu6")
def convert_relu6(g: GraphBuilderExtendedProtocol, sts: Dict[str, Any], outputs: List[str], op, verbose: int = 0) -> str:
...
def convert_sigmoid(g: GraphBuilderExtendedProtocol, sts: Dict[str, Any], outputs: List[str], op) -> str:
    """TF ``Sigmoid`` → ONNX ``Sigmoid``."""
    return g.op.Sigmoid(op.inputs[0].name, outputs=outputs, name=op.name)


@register_tf_op_converter("Tanh")
def convert_tanh(g: GraphBuilderExtendedProtocol, sts: Dict[str, Any], outputs: List[str], op) -> str:
    """TF ``Tanh`` → ONNX ``Tanh``."""
    return g.op.Tanh(op.inputs[0].name, outputs=outputs, name=op.name)


@register_tf_op_converter("Softmax")
def convert_softmax(g: GraphBuilderExtendedProtocol, sts: Dict[str, Any], outputs: List[str], op, verbose: int = 0) -> str:
