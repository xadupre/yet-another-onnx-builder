...
class FakeTensorContext:
    """Stores information used to reuse the same dimension for the same dimension names."""
    ...
    def make_fake(self, x: Any) -> Optional["FakeTensor"]:  # noqa: F821
        """See :func:`onnx_pipe.tracing.fake_tensor.make_fake`."""
        if x is None:
            return None
        ...
        if x.__class__.__name__ in {"DynamicCache", "StaticCache", "HybridCache"}:
            assert hasattr(x, "layers"), (
                f"Use more recent version of transformers (>=4.55), "
                f"'layers' not found in class {type(x)}"
            )
            for layer in x.layers:
                assert hasattr(layer, "keys") and hasattr(layer, "values"), (
                    f"Use more recent version of transformers (>=4.55), 'layers' "
        ...
        raise TypeError(f"Unexpected type {type(x)} for x, content is {repr(x)}")

    def make_fake_with_dynamic_dimensions(self, x: Any, dynamic_shapes: Any) -> Any:
        """
        See
        :func:`onnx_pipe.tracing.fake_tensor.make_fake_with_dynamic_dimensions`.
        ...
        if x.__class__.__name__ == "BaseModelOutput":
            assert (
                list(x.keys()) == ["last_hidden_state"]
                and x.last_hidden_state is not None
            ), (
...
def make_fake(
    ...
    .. runpython::
        ...
        from onnx_pipe.tracing.fake_tensor import make_fake
...
def make_fake_with_dynamic_dimensions(
    x: Any,
    dynamic_shapes: Any,
    context: Optional["FakeTensorContext"] = None,  # noqa: F821
    ...
    This uses function :func:`onnx_diagnostic.helpers.fake_tensor.make_fake`.
    ...
    .. runpython::
        ...
        import torch
        from onnx_pipe.tracing.fake_tensor import make_fake_with_dynamic_dimensions
