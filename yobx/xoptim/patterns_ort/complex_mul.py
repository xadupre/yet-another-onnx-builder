...
class _ComplexMulPatternBase(PatternOptimization):
    """Base class providing shared helpers for complex multiplication fusion patterns."""

    @classmethod
    def _get_gather_source(
        cls, g: "GraphBuilderPatternOptimization", name: str, expected_index: int  # noqa: F821
    ) -> Optional[str]:
    ...
    def _is_unsqueeze_last_dim(
        cls, g: "GraphBuilderPatternOptimization", node: Optional[NodeProto]  # noqa: F821
    ) -> bool:
    ...
    def _classify_mul(
        ...
        for inp1, inp2 in [(node.input[0], node.input[1]), (node.input[1], node.input[0])]:
            for a_idx in (0, 1):
                a_src = cls._get_gather_source(g, inp1, a_idx)
                if a_src != src_a:
                    continue
                for b_idx in (0, 1):
                    b_src = cls._get_gather_source(g, inp2, b_idx)
                    if b_src != src_b:
                        continue
                    return (a_idx, b_idx)
        return None
    ...
    def _find_gather_node(
        ...
        for inp in mul_node.input:
            if cls._get_gather_source(g, inp, idx) == src:
    ...
    def _parse_complex_mul_core(
        ...
        Returns
        ``(A, B, gather_a0, gather_a1, gather_b0, gather_b1, mul_rr, mul_ii, mul_ri, mul_ir)``
        ...
        def gather_sources(mul_node):
            ...
            for inp in mul_node.input[:2]:
                for idx in (0, 1):
                    src = cls._get_gather_source(g, inp, idx)
                    if src is not None:
                        srcs.add(src)
                        break
            return srcs
        ...
        candidates = (
            [(sources[0], sources[1]), (sources[1], sources[0])] if len(sources) == 2 else []
        )
        if len(sources) == 1:
        ...
        for A, B in candidates:
            if real_sub_imag_add:
                ...
                r1 = cls._classify_mul(g, real_node.input[0], A, B)
                r2 = cls._classify_mul(g, real_node.input[1], A, B)
                if r1 != (0, 0) or r2 != (1, 1):
                    continue
                # imag Add can have either ordering of ri/ir
                i_opts = [
                    (
                        cls._classify_mul(g, imag_node.input[0], A, B),
                        cls._classify_mul(g, imag_node.input[1], A, B),
                    )
                ]
                valid_imag = None
                for ia, ib in i_opts:
                ...
                r_opts = [
                    (
                        cls._classify_mul(g, real_node.input[0], A, B),
                        cls._classify_mul(g, real_node.input[1], A, B),
                    )
                ]
                valid_real = None
                for ra, rb in r_opts:
                ...
                i1 = cls._classify_mul(g, imag_node.input[0], A, B)
                i2 = cls._classify_mul(g, imag_node.input[1], A, B)
            ...
            g_a0 = cls._find_gather_node(g, mul_rr, A, 0)
            g_a1 = cls._find_gather_node(g, mul_ii, A, 1)
            g_b0 = cls._find_gather_node(g, mul_rr, B, 0)
            g_b1 = cls._find_gather_node(g, mul_ii, B, 1)
...
class ComplexMulPattern(_ComplexMulPatternBase):
    """
    ...
    def match(
        ...
        if not self._is_unsqueeze_last_dim(g, unsq_r):
            return self.none(node, inspect.currentframe().f_lineno)
        if not self._is_unsqueeze_last_dim(g, unsq_i):
            return self.none(node, inspect.currentframe().f_lineno)
        ...
        result = self._parse_complex_mul_core(g, real_node, imag_node, real_sub_imag_add=True)
        if result is None:
            return self.none(node, inspect.currentframe().f_lineno)
...
class ComplexMulConjPattern(_ComplexMulPatternBase):
    """
    ...
    def match(
        ...
        result = self._parse_complex_mul_core(g, real_node, imag_node, real_sub_imag_add=False)
        if result is None:
            return self.none(node, inspect.currentframe().f_lineno)
