...
@register_sklearn_converter(SelectFpr)
def sklearn_select_fpr(
    ...
    """
    Converts a :class:`sklearn.feature_selection.SelectFpr` into ONNX.

    After fitting, the transformer keeps the features whose p-value is below
    the false positive rate threshold ``alpha``.  The selected column indices
    ...
    :param estimator: a fitted ``SelectFpr``
    :param outputs: desired output names
    :param X: input name (shape ``(N, F)``)
    :param name: prefix name for the added nodes
    :return: output name
    """
    assert isinstance(estimator, SelectFpr), f"Unexpected type {type(estimator)} for estimator."
