...
def _ndtri_approx(g, p, dtype, name):
    ...
    Uses Acklam's algorithm (max absolute error ≈ 1.15 × 10⁻⁹).
...
def _interp_1d_batch(g, X, q_T_const, r_const, dtype, name):
    ...
    feature_offset = (
        np.arange(n_features, dtype=np.int64) * n_quantiles
    ).reshape(1, n_features)                    # (1, F) constant
...
def sklearn_quantile_transformer(
    ...
    **Normal distribution** output uses Acklam's rational-function
    approximation of the inverse normal CDF (max error ≈ 1.15 × 10⁻⁹).
    ...
    * opset ≥ 13 – ``ReduceSum`` with axes as input tensor
    ...
    if estimator.output_distribution == "uniform":
        ...
        clip_min = float(np.float64(
            _acklam_scalar(BOUNDS_THRESHOLD - np.spacing(np.float64(1)))
        ))
        clip_max = float(np.float64(
            _acklam_scalar(1.0 - (BOUNDS_THRESHOLD - np.spacing(np.float64(1))))
        ))
        res = g.op.Clip(
            ndtri_out,
