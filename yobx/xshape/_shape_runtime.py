...
class _ShapeRuntime:
    ...
    def simple_update_value_shape_with_node(self, node: onnx.NodeProto) -> bool:
        ...
        meth_name = f"_update_value_shape_with_values_{node.op_type}"
        if hasattr(self, meth_name):
    ...
    def _update_value_shape_with_node_Identity(self, node: onnx.NodeProto) -> bool:
        ...
        if value is not None:
            ...
            self.set_value_shape(
                ...
                (
                    np.abs(value)
                    if node.op_type == "Abs" and all(isinstance(s, (int, float)) for s in value)
                    else value
                ),
    ...
    def _update_value_shape_with_values_Add(
        ...
        return self._update_value_shape_with_values_element_wise(node, values)

    def _update_value_shape_with_values_Mul(
        self, node: onnx.NodeProto, values: List[Any]
    ) -> bool:
        return self._update_value_shape_with_values_element_wise(node, values)

    def _update_value_shape_with_values_Sub(
        self, node: onnx.NodeProto, values: List[Any]
    ) -> bool:
        return self._update_value_shape_with_values_element_wise(node, values)

    def _update_value_shape_with_values_Div(
        self, node: onnx.NodeProto, values: List[Any]
    ) -> bool:
        return self._update_value_shape_with_values_element_wise(node, values)

    def _update_value_shape_with_values_Mod(
        self, node: onnx.NodeProto, values: List[Any]
    ) -> bool:
        return self._update_value_shape_with_values_element_wise(node, values)

    def _update_value_shape_with_values_element_wise(
        self, node: onnx.NodeProto, values: List[Any]
    ) -> bool:
        fct, symbol = {
            "Add": ((lambda x, y: x + y), "+"),
    ...
    def _update_value_shape_with_values_Gather(
        self, node: onnx.NodeProto, values: List[Any]
    ) -> bool:
